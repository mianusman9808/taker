<?php
	...
	class  excelgen {
	     ...
	     // Default constructor
	     //PAGUIAR: contructur OLD
	    function  iniciar($excel_filename='excelgen')
	     {
        ...
        function ExcelOutput($result,$nombreArchivo)
        {

            $excel = $this->iniciar($nombreArchivo);
            ...
            foreach ($resultado as $nombre=>$field){
               
                $this->WriteText($rowscounter,$colscounter,$nombre);
                 $colscounter++; 
             }
            ...
            foreach ($result as $resultado){
                ...
                foreach ($resultado as $field){
                    if(is_numeric($field)){
                        $this->WriteNumber($rowscounter,$colscounter,$field);
                    }else{
                        $this->WriteText($rowscounter,$colscounter,utf8_decode($field)); 
                    }
                    $colscounter++;
                }
                $rowscounter++;
